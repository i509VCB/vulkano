...
use vulkano::Version;
use vulkano::swapchain::display::{DisplayMode, Display, DisplayPlane};
...
fn main() {
	...
	let required_extensions = InstanceExtensions {
		khr_display: true, // VK_KHR_display
		khr_surface: true, // Required by above
	...
	let display = Display::enumerate(physical).next().unwrap();

	let physical_resolution = display.physical_resolution();
	// Find the first suitable display mode at the resolution.
	let display_mode = display.display_modes()
		.find(|mode| mode.visible_region() == physical_resolution)
		.unwrap();

	let plane = DisplayPlane::enumerate(physical).find(|plane| plane.supports(&display))
		.unwrap();
	...
	let surface = Surface::<()>::from_display_mode(&display_mode, &plane).unwrap();
	...
	loop {
		...
		// Well we don't have a great frame timer...
		let ms = 1000 as f32 / display_mode.refresh_rate() as f32;
